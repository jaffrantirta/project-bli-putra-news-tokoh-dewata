...
export default function NewsList() {
  ...
  useEffect(() => {
    ...
    async function getSearch() {
      const { data, error } = await showLike(
        "title",
    ...
    async function getNewsByCategory() {
      const { data, error } = await show()
        .eq("category_id", queryParams.get("id"))
        .select("*, categories(*)")
        .range(0, 10);
      if (error) {
        Swal.fire(ERROR_MESSAGE, error.message, "error");
  ...
  return (
    <div>
      ...
      <hr></hr>
      <NavbarSection />
      {queryParams.has("keyword") ? (
        <p className="text-center md:text-4xl px-10 underline">
          Menampilkan pencarian{" "}
          <span className="font-bold">"{queryParams.get("keyword")}"</span>
        </p>
      ) : (
        <></>
      )}
      {queryParams.has("id") ? (
        <p className="text-center md:text-4xl px-10 underline">
          Menampilkan berita kategori{" "}
          <span className="font-bold">"{queryParams.get("name")}"</span>
        </p>
      ) : (
        <></>
      )}
      {queryParams.has("tag") ? (
        <p className="text-center md:text-4xl px-10 underline">
          Menampilkan berita dengan tag{" "}
          <span className="font-bold">"{queryParams.get("tag")}"</span>
        </p>
      ) : (
        <></>
      )}
      <div className="p-10 grid grid-cols-q md:grid-cols-4 gap-10">
        <div className="md:col-span-3">
          {loading ? (
            <Loader loadText={"Mohon tunggu..."} />
          ) : newsListLeft.length === 0 ? (
            <Message img={ImgNotFound} message={"Tidak ada berita ditemukan"} />
          ) : (
